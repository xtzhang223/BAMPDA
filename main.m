...
% adaj_dm=zeros(nd,nv);
% for i=1:pp
%         adaj_dm(A(i,1),A(i,2))=1;                                           %adaj_dm: 已知联系的置1
% end
...
for k=58:number
    ...
    for circle=0:4
            ...
            [protein_integration_similarity,disease_integration_similarity] = integration_protein_disease_similarity(protein_gauss, disease_gauss ); %调用函数获得整合药物副作用，药物化学结构相似性，高斯相似性的药物相似性矩阵，病毒相似性也是如此
            
            %调用三种方法获取得分矩阵
%             [predict_score_matrix1,numhid]=RBM_model(interaction_matrix1);   %调用RBM方法，numhid是number of hidden layers
%             [predict_score_matrix_protein,threshold]=integrated_neighbor_model_protein( interaction_matrix1,protein_integration_similarity);   %整合的领域的方法,用药物-病毒相似矩阵和药物相似矩阵
